...
function genColumns(fields: ContentFieldsMappingDto[], filterFields: any[], typeName: string, gpFields: string) {
        ...
        if (field.fieldType)
            switch (field.fieldType) {
                case FieldType.TextField:
                    ...
                    if (fieldSettings?.ContentPartFieldSettings?.Editor == "PredefinedList") {
                        ...
                        if (fieldSettings?.TextFieldPredefinedListEditorSettings?.Options) {
                            item.options =
                                fieldSettings?.TextFieldPredefinedListEditorSettings?.Options.map((x: any) => {
                                    return {
                                        label: x.name,
                                        value: x.value
                                    }
                ...
                case FieldType.UserPickerField:
                    item.name = field.graphqlValuePath?.replace('userIds.firstValue', 'firstUserProfiles.displayText')
                    item.autoComplete = {
                        method: "post",
                        url: "/api/graphql",
                        dataType: "json",
            ...
            case FieldType.TextField:
                ...
                if (fieldDef.fieldSettings?.ContentPartFieldSettings?.Editor == "PredefinedList") {
                    ...
                    if (fieldDef.fieldSettings?.TextFieldPredefinedListEditorSettings?.Options) {
                        field.options =
                            fieldDef.fieldSettings?.TextFieldPredefinedListEditorSettings?.Options.map((x: any) => {
                                return {
                                    label: x.name,
                                    value: x.value
                                }
