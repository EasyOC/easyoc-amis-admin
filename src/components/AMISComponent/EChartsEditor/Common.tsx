 ...
 * @param label
 */
export const color = (name: string, label: string, defaultColor?: string, labelRemark?: string) => {
  return {
    type: 'color',
    format: 'rgba',
    pipeIn: (value: any) => {
      if (typeof value === 'undefined' && defaultColor && defaultColor !== 'null') {
        return defaultColor;
      }
      return value;
    },
 ...
 * @param label
 */
export const trueSwitch = (name: string, label: string, labelRemark?: string) => {
  return {
    type: 'switch',
    name: name,
    mode: 'inline',
 ...
 * @param label
 */
export const falseSwitch = (name: string, label: string, labelRemark?: string) => {
  return {
    type: 'switch',
    name: name,
    mode: 'inline',
 ...
 * @param options
 */
export const select = (name: string, label: string, options: any[], labelRemark?: string) => {
  return {
    type: 'select',
    name: name,
    label: label,
 ...
 * @param collapsed
 */
export const fieldSet = (label: string, controls: any[], collapsed: boolean = false) => {
  return {
    type: 'fieldSet',
    title: label,
    collapsable: true,
 ...
 * @param defaultText
 */
export const text = (name: string, label: string, labelRemark?: string, defaultText?: string) => {
  const control: any = {
    type: 'input-text',
    name: name,
    label: label,
...
export const viewport = (scope: string, label: string) => {
  return {
    ...
    mountOnEnter: true,
    body: [
      ...viewportControl(`${scope}left`, `${label}离容器左侧的距离`, '左侧距离值类型', [
        'auto',
        'left',
        'center',
        'right',
      ]),
      ...viewportControl(`${scope}top`, `${label}离容器上侧的距离`, '上侧距离值类型', [
        'auto',
        'top',
        'middle',
        'bottom',
      ]),
      ...viewportControl(`${scope}right`, `${label}离容器右侧的距离`, '右侧距离值类型', ['auto']),
      ...viewportControl(`${scope}bottom`, `${label}离容器下侧的距离`, '下侧距离值类型', ['auto']),
    ],
  };
};
...
export const numberOrArray = (
  ...
  return [
...

    {
      ...
      name: name,
      label: label,
      labelRemark: '设置两个值将分别是上下、左右；设置四个值则分别是上、右、下、左',
  ...
  return {
    ...
    body: [
      ...
      },
      {
        type: 'input-text',
        name: name,
        visibleOn: `this.${name} && ${JSON.stringify(keywordList)}.indexOf(this.${name}) === -1`,
        label: label,
      },
      {
        type: 'select',
 ...
 * @param controls
 */
export const objectOrArray = (name: string, labelForSwitch: string, controls: any[]) => {
  return [
    {
      type: 'switch',
      label: labelForSwitch,
...
export const commonStyle = (scope: string, label: string) => {
  return [
    ...
    ...numberOrArray(`${scope}borderRadius`, '圆角半径', '单独设置每个圆角半径'),
...
export const booleanOrKeyword = (
  ...
  return {
    ...
    options: [{ label: '开启', value: 'true' }, { label: '关闭', value: 'false' }, ...keywordList],
  };
};
  ...
  return createHierarchy(scope, [
    {
      ...
      body: [
        ...
        select('fontStyle', `${label}文字字体的风格`, ['normal', 'italic', 'oblique']),
        keywordOrNumber('fontWeight', `${label}文字字体的粗细`, '字体粗细格式使用数字', [
          'normal',
          'bold',
          'bolder',
          'lighter',
...
const buildOneOption = (scope: string, name: string, option: any) => {
  ...
  let remark = descSplit.length > 1 ? descSplit[1].trim().replace('</p>', '') : '';
  // 有些描述太长了，再通过逗号拆分一下
  ...
  if (uiControlType === 'boolean') {
    ...
    const defaultValue = typeof uiControl.default === 'undefined' ? 0 : +uiControl.default;
...
export const buildGroupOptions = (scope: string, parentName: string, options: any) => {
  let controls = [];
...
export const buildOptions = (scope: string, options: any) => {
  ...
  for (const name in options) {
    ...
    // }
    // 这些样式单独处理或忽略
    if (commonStyleKeys.has(name) || viewportKeys.has(name) || uselessKeys.has(name)) {
      continue;
    }
    if (
    ...
    if (groupKeys.has(name)) {
      controls.push(fieldSet(name, buildGroupOptions(scope, name, options), true));
    }
